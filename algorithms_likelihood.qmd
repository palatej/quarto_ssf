...
So, by concentrating the scaling factor $\sigma^2$ out of the likelihood, we get, using the notations defined for the ordinary filter:
...
$$c_t= Z_t\left[ \left(\prod_{i=0}^{i<t}{T_i}\right)\right]a0, \quad X_t = Z_t\left[ \left(\prod_{i=0}^{i<t}{T_i}\right)\right]B $$ and the covariance matrix $\sigma^2\Omega$ of $\mu$ depends on the structure of the state space model
